...
export const createSession = async (req, res) => {
  try {
    ...
    sendResponse(
      res,
      400,
      "Requête incorrecte : Impossible de créer une session."
    );
  }
};
...
export const leaveSession = async (req, res) => {
  try {
    ...
    const connectedUsers = await db
      ...
      .get();
    console.log("test", connectedUsers);
    if (!connectedUsers) {
    ...
    sendResponse(
      res,
      400,
      "Requête incorrecte : Impossible de déconnecter une session."
    );
  }
};
...
export const joinSession = async (req, res) => {
  try {
    ...
    if (!session) {
      return sendResponse(res, 404, "Non trouvé : Session introuvable.");
    }
    ...
    sendResponse(
      res,
      400,
      "Requête incorrecte : Impossible de rejoindre une session."
    );
  }
};
  ...
  try {
    ...
    if (!roomKey) {
      ...
      return sendResponse(
        res,
        400,
        "Requête incorrecte : La clé de la salle est requise."
      );
    }
    const session = await db
      .select()
